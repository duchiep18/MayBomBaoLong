        ...
        <div class="container">
            <div class="row">
                <div class="col-md-10">
                    ...
                    <form action="{{route('products.update', $addnew->id)}}" method="POST" role="form">
                        @csrf
                        @method('put')
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <select name="loaisanpham" class="form-control show-tick"  >
                                            ...
                                            @foreach($categories_prd as $category_prd)
                                                <option {{ $category_prd->id == $addnew->categories_prd_id ? 'selected' : '' }} value="{{$category_prd->id}}"> {{$category_prd->tendanhmucsp}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" value="{{$addnew->tensanpham}}" name="tensanpham" class="form-control" id="" placeholder="">
                        </div>

                        <div class="form-group">
                            <label for="">Thông tin sản phẩm:</label>
                            <textarea name="thongtinsanpham" id="input" class="form-control" rows="10" required="required">{{$addnew->thongtinsanpham}}</textarea>

                        </div>

                        <div class="form-group">
                            <label for="">Giá sản phẩm:</label>
                            <input type="text" value="{{$addnew->giasanpham}}"  name="giasanpham" class="form-control"  id="" placeholder="">
                        </div>

                        <div class="form-group">
                            <label for=""> Ảnh sản phẩm:</label>
                            <img src="{{$addnew->image}}" alt="">
                        </div>
